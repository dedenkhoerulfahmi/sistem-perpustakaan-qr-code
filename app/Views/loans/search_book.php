          ...
          <table>
            <?php foreach ($tableData as $key => $value) : ?>
              <tr>
                ...
                <td>
                  <h6 class="text-black-50"><?= esc($value); ?></h6>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
...
<script>
  function getBookData(param) {
    jQuery.ajax({
      ...
      success: function(response, status, xhr) {
        ...
        $('html, body').animate({ scrollTop: $("#bookResult").offset().top }, 500);
      },
  ...
  function selectBook({ slug, title, cover, stock }) {
    if (!bookSelection.has(slug) && bookListElement.querySelector(`#${slug}`) === null) {
      const book = { slug, title, cover, stock };
          ...
          <div class="card border border-2 border-primary overflow-hidden position-relative" style="height: 160px; max-width: 355px;">
            <div class="card-body">
              <div class="position-absolute top-50 start-0 translate-middle-y border border-black me-4"  
                   style="background-image: url(<?= base_url(BOOK_COVER_URI); ?>${book.cover}); height: 160px; width: 120px; background-position: center; background-size: cover;">
              </div>
